...
Speed CalSpeed(Coordinate start_point, Coordinate end_point, Quaternion quaternion)
{
    ...
    double target_theta = atan2(delta_y, delta_x);

    double self_theta = GetRPY(quaternion).yaw;
    double delta_theta = target_theta - self_theta;
    delta_theta = fmod(delta_theta + M_PI, 2 * M_PI) - M_PI;

    Speed return_speed;
    return_speed.angular = std::min(10 * PROPORTIONAL_COEFFICIENT * delta_theta, ANGULAR_SPEED_MAX);
