...
GlobalMap::GlobalMap(std::string name) : Node(name)
{
    ...
    this->SubscriptionTF = this->create_subscription<geometry_msgs::msg::TransformStamped>("/tf",10,std::bind(&GlobalMap::SubTF,this,_1));

    // timer
    this->MapTimer = this->create_wall_timer(std::chrono::seconds(1),std::bind(&GlobalMap::PubMap,this));
}

void GlobalMap::SubTF(const geometry_msgs::msg::TransformStamped::SharedPtr info)
{
    if (info->header.frame_id != "map") return;
    std::string sub_frame = info->child_frame_id;
    ...
    this->RobotTF[std::stoi(id)].tf.x = info->transform.translation.x;
    this->RobotTF[std::stoi(id)].tf.y = info->transform.translation.y;
}
...
void GlobalMap::SubMap(const nav_msgs::msg::OccupancyGrid::SharedPtr info)
{
    ...
    origin.position.x = info->info.origin.position.x;
    origin.position.y = info->info.origin.position.y;
    // 更新地图
    ...
    else
    {
        ...
        int left = std::ceil((this->map.origin.x - info->info.origin.position.x)/MAP_RESOLUTION);
        int right = std::ceil(this->map.origin.x/MAP_RESOLUTION + this->map.width - info->info.origin.position.x/MAP_RESOLUTION - info->info.width);
        int down = std::ceil((this->map.origin.y - info->info.origin.position.y)/MAP_RESOLUTION);
        int up = std::ceil(this->map.origin.y/MAP_RESOLUTION + this->map.height - info->info.origin.position.y/MAP_RESOLUTION - info->info.height);
